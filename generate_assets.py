...
PROJECT_COLORS: dict[str, tuple[str, str]] = {
    "langdepth": ("#1D4E89", "#3776AB"),
    "llama3-rag": ("#B02A1B", "#EE4C2C"),
    "legalrag": ("#0F766E", "#14B8A6"),
    "visionshopper": ("#C2410C", "#EA580C"),
    "adadepthclip": ("#1F4E6D", "#336791"),
    "fairseq-en-fa": ("#00796B", "#009688"),
    "multinli": ("#CC7A00", "#FF9900"),
    "ngs-data-analyser": ("#047857", "#10B981"),
    "diabet-xai": ("#BE123C", "#E11D48"),
    "bachelors-thesis": ("#004482", "#00599C"),
    "arxiv-paper": ("#1D4E89", "#2563EB"),
    "website-link": ("#0891B2", "#06B6D4"),
}

SOCIAL_COLORS: dict[str, tuple[str, str]] = {
    "linkedin": ("#0A66C2", "#0A66C2"),
    "email": ("#DC2626", "#EF4444"),
    "website": ("#0891B2", "#06B6D4"),
    "github": ("#24292F", "#374151"),
}

LUCIDE_STROKE = "#475569"
...
def write_text_logo(name: str, label: str, color_start: str, color_end: str) -> None:
...
def write_icon(name: str, icon_key: str, color_start: str, color_end: str) -> None:
    path = ICON_PATHS[icon_key]
    grad_id = f"grad-{name}-icon"
    svg = textwrap.dedent(
        f"""\
        <svg xmlns="http://www.w3.org/2000/svg" width="22" height="22" viewBox="0 0 24 24">
          <defs>{gradient_def(grad_id, color_start, color_end)}</defs>
          <g fill="none" stroke="url(#{grad_id})" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">{path}</g>
        </svg>
        """
    ).strip()
...
def lucide_icon(name: str, path: str) -> None:
    svg = textwrap.dedent(
        ...
        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24">
          <g fill="none" stroke="{LUCIDE_STROKE}" stroke-linecap="round" stroke-linejoin="round" stroke-width="2">{path}</g>
        </svg>
        """
    ).strip()
...
def main() -> None:
    ...
    social_icons = {
        ...
        "website": '<circle fill="white" cx="12" cy="12" r="9"/><path fill="none" stroke="white" stroke-width="2" d="M2 12h20"/><path fill="none" stroke="white" stroke-width="2" d="M12 2a15 15 0 0 1 0 20"/><path fill="none" stroke="white" stroke-width="2" d="M12 2a15 15 0 0 0 0 20"/>',
    ...
    for slug, label, icon_key in projects:
        start, end = PROJECT_COLORS[slug]
        write_text_logo(slug, label, start, end)
        write_icon(slug, icon_key, start, end)
